...
- **Red Grid Point Filtering**: Red grid points are now rendered in the PDF only if they are within Euclidean distance $\le 5$ of a differently colored grid point (yellow origin or blue good set point) or within distance $\le 5$ of the grid bounding box boundary.
