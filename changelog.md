...
- **System Hardware, Execution Start Time & Hostname Metadata (`sys_info.rs`)**: Execution metadata (`# Start Time`, `# Hostname`, `# CPU Model`, `# Total Memory`, and `# Operating System`) is now dynamically gathered and included in the `#` summary section printed to standard output, exported `.txt` files, and rendered in the PDF info section.
...
- **Cleaned-up Polygon Storage & Linear $O(V)$ Algorithm**: Replaced the legacy multi-pass polygon cleanup function with a single-pass $O(V)$ stack-based algorithm (`polygon_rm_redundant_vertices` in `geom.rs`) that pops redundant collinear vertices during insertion and handles wraparound boundary collinearity. Added dedicated unit tests for all edge cases.
