...
- **System Hardware & OS Info (`sys_info.rs`)**: System metadata (`# CPU Model`, `# Total Memory`, and `# Operating System`) is now dynamically gathered and included in the `#` summary section printed to standard output, exported `.txt` files, and rendered in the PDF info section.
